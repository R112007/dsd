package dsd.world.blocks.production;
...
public class DrillTurret extends BaseTurret {
  ...
  public class DrillTurretBuild extends BaseTurretBuild {
    ...
    public void updateTile() {
      ...
      // target ore
      targetMine(core);
      if (core == null || mineTile == null || !canConsume() || !Angles.within(rotation, angleTo(mineTile), shootCone)
    ...
    public @Nullable Item iterateMap(Building core) {
      ...
      for (int i = 0; i < proxOres.size; i++) {
        if (canMine(proxItems.get(i))
            && (last == null || last.lowPriority || core.items.get(last) > core.items.get(proxItems.get(i)))) {
